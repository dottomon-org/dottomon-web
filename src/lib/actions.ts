...
import { SHEET_POSES, assetReadme, buildZip, toAssetZip, toGif, toPng, toSpriteSheet, type ZipEntry } from "@dotmon/core/render";
...
export async function downloadFavoritesZip(
  ...
  for (const f of favs) {
    ...
    usedDirs.add(dir);
    files.push({ name: `${dir}/README.txt`, data: new TextEncoder().encode(assetReadme(f.seed, locale)) });
    for (const view of VIEWS) {
      for (const [pose, frame] of SHEET_POSES) {
        const png = await toPng(generateSvg(f.seed, { ...f.opts, view, frame }).svg, { size: 512, background });
        files.push({ name: `${dir}/${view}_${pose}.png`, data: new Uint8Array(await png.arrayBuffer()) });
      }
      const gif = await toGif(
        [generateSvg(f.seed, { ...f.opts, view, frame: 1 }).svg, generateSvg(f.seed, { ...f.opts, view, frame: 2 }).svg],
        { size: 256, background },
      );
