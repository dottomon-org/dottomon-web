...
export default function MainPreview(p: Props) {
  return (
    <section className={`${panel} grid grid-cols-[210px_1fr] items-center gap-4.5 max-md:gap-3 max-sm:grid-cols-1 max-sm:justify-items-center`}>
      <div className="relative grid size-[210px] place-items-center rounded-[10px] border border-line" style={{ background: bgStyle(p.bg, 20) }}>
      ...
      <div className="min-w-0 text-[12.5px] text-dim max-sm:w-full max-sm:px-4 [&>*+*]:mt-2">
        <div className="flex min-w-0 items-baseline gap-2">
          ...
          <span className="min-w-0 flex-[0_1_auto] overflow-hidden text-[19px] font-bold tracking-[0.02em] text-ellipsis whitespace-nowrap text-acid" title={p.seed}>
            {p.seed}
          </span>
        </div>
