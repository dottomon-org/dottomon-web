...
import { checkboxCls, colorInputCls, focusRing, panelH2, selectCls, trow } from "../lib/ui";
...
export default function Sidebar(p: Props) {
  return (
    <aside className={`${ASIDE} ${p.open ? "max-md:translate-x-0" : "max-md:-translate-x-[105%]"}`} id="sidebar">
      <button
            ...
            return (
              <button
                ...
                className={`grid w-full cursor-pointer grid-cols-[54px_1fr] items-center gap-2.5 rounded-[10px] border-2 bg-panel2 px-2.5 py-2.25 text-left font-mono ${
                  sel ? "border-acid shadow-[0_0_0_3px_rgba(184,245,66,0.12)]" : "border-line hover:border-dim"
                } ${focusRing}`}
                type="button"
              ...
              >
                ...
                <span>
                  <span className={`text-[13px] font-bold ${sel ? "text-acid" : "text-ink"}`}>{p.dict.presets[k]}</span>
                  <br />
                  <span className="text-[10px] leading-normal text-dim">{p.dict.presetDescriptions[k]}</span>
                </span>
              </button>
            );
          })}
        ...
        <div className="grid">
          {p.preset !== "chaos" && (
            <label className={trow}>
              <input type="checkbox" className={checkboxCls} checked={p.tweaks.outline} onChange={(e) => p.onTweaks({ ...p.tweaks, outline: e.target.checked })} />
              {p.t.outline}
            </label>
          )}
          {p.preset !== "chaos" && (
            <label className={trow}>
              <input type="checkbox" className={checkboxCls} checked={p.tweaks.face} onChange={(e) => p.onTweaks({ ...p.tweaks, face: e.target.checked })} />
              {p.t.face}
            </label>
          )}
          {p.preset !== "mochi" && (
            <label className={trow}>
              <input type="checkbox" className={checkboxCls} checked={p.tweaks.gapFill} onChange={(e) => p.onTweaks({ ...p.tweaks, gapFill: e.target.checked })} />
              {p.t.gapFill}
            </label>
          )}
          {p.preset === "mochi" && (
            <div className={trow}>
              ...
              <select id="o-legs" className={selectCls} value={p.tweaks.legs} onChange={(e) => p.onTweaks({ ...p.tweaks, legs: e.target.value as Legs })}>
                {LEG_OPTIONS.map((l) => (
      ...
      <section>
        ...
        <div className="grid">
          <label className={trow}>
            <input type="checkbox" className={checkboxCls} checked={p.animate} onChange={(e) => p.onAnimate(e.target.checked)} />
            {p.t.animation}
          </label>
          <div className={trow}>
            <span>{p.t.background}</span>
            <label className="flex items-center gap-[5px]">
              <input type="checkbox" className={checkboxCls} checked={p.bgTrans} onChange={(e) => p.onBgTrans(e.target.checked)} />
              {p.t.transparent}
            </label>
            <input
              type="color"
